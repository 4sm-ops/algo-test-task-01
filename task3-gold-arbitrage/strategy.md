...
### 11.5 Лимитные ордера как альтернатива market orders

Текущая реализация использует **market orders** (crossing spread) — это worst-case сценарий, при котором мы теряем полный bid-ask spread на каждой ноге.

#### Идея

Вместо пересечения спреда на B3 — выставлять **limit order внутри спреда**:

```
Текущий подход (market order):
  BUY B3 @ ask = 2740.0    (теряем 26 пт на crossing)

Лимитный ордер:
  BUY B3 @ mid = 2727.0    (теряем ~0 пт, но ждём исполнения)
  BUY B3 @ bid + 5 = 2719.0 (зарабатываем на спреде, но низкий fill rate)
```

На MOEX оставляем market order — спред узкий (~2 пт), потери минимальны.

#### Механика исполнения

| Нога | Тип ордера | Цена | Потери на спреде |
|------|-----------|------|-----------------|
| **B3 (текущая)** | Market | ask / bid | ~13 пт за ногу |
| **B3 (limit @ mid)** | Limit | (bid+ask)/2 | ~0 пт за ногу |
| **MOEX** | Market | ask / bid | ~1 пт за ногу |

#### Ожидаемый эффект

Round-trip потери при limit orders на B3:

```
Market orders:  13 + 1 + 13 + 1 = 28 пт (текущее)
Limit @ mid:     0 + 1 +  0 + 1 =  2 пт (оптимистичное)
Limit @ bid+5:  -8 + 1 + -8 + 1 = нет потерь + потенциальный заработок
```

Даже при заполнении по mid-price потери сокращаются с **28 пт до 2 пт** — это кардинально меняет экономику стратегии.

#### Риски лимитных ордеров

1. **Fill rate** — лимитный ордер может не исполниться. При спреде 26 пт и низкой ликвидности B3 fill rate может быть < 50%
2. **Adverse selection** — ордер исполняется когда цена движется против нас (маркет-мейкер проблема)
3. **Timing risk** — пока ждём исполнения на B3, цена на MOEX может уйти, и сигнал станет неактуальным
4. **Partial fills** — частичное исполнение создаёт незахеджированную позицию
5. **Latency** — при задержке 250 мс на B3 лимитный ордер может быть выставлен по уже неактуальной цене

#### Практическая реализация

Для бэктеста лимитных ордеров потребуется:
- Модель fill rate в зависимости от расстояния до best bid/ask
- Очередь ордеров и моделирование времени исполнения
- Обработка случаев неисполнения (cancel/replace logic)

Это существенно усложняет бэктест, но является необходимым шагом для реалистичной оценки стратегии.
